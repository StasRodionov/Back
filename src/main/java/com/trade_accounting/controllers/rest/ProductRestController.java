...
@Slf4j
@RestController
@Tag(name = "Product Rest Controller", description = "CRUD операции с продуктами")
...
public class ProductRestController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(code = 201, message = "Запрос принят и товар успешно создан"),
            @ApiResponse(code = 401, message = "Нет доступа к данной операции"),
            @ApiResponse(code = 403, message = "Операция запрещена"),
            @ApiResponse(code = 404, message = "Данный контроллер не найден")
    })
    ...
    @ApiOperation(value = "create", notes = "Обновляет товар на основе переданных данных")
    @PutMapping
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Товар успешно обновлен"),
            @ApiResponse(code = 201, message = "Запрос принят и товар успешно обновлен"),
            @ApiResponse(code = 401, message = "Нет доступа к данной операции"),
            @ApiResponse(code = 403, message = "Операция запрещена"),
            @ApiResponse(code = 404, message = "Данный контроллер не найден")
    })
